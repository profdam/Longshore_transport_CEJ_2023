...
aa=isfinite(hs_mean); hs=hs_mean(aa); dm=dm_mean(aa); tm=tm_mean(aa);  aalon=lon_mean(aa); aalat=lat_mean(aa);

alon=round(aalon,3);
indlon = find(alon ==8.819  | alon == 8.941 | alon == 9.115);
aalon(indlon)=nan; aalat(indlon)=nan; hs(indlon)=nan; tm(indlon)=nan; dm(indlon)=nan;
aa=isfinite(hs); hs=hs(aa); tm=tm(aa); dm=dm(aa); aalon=aalon(aa); aalat=aalat(aa);

%% 
m_proj('mercator','lon',[lon1x lon2x],'lat',[lat1y lat2y]);
figure
% m_scatter(lon_mean,lat_mean,50,hs_mean,'fill')
m_scatter(aalon,aalat,50,hs,'fill')
hold on
colormap(jet)
%m_plot(lon_css,lat_css,'.k')
m_coast('color',[.1 .1 .1])
