...
cd D:\Dossa\SSS\data\SMOS
load('SSS_SMOS_LOCEAN_18days')
% define parameters
DD = 250;  % distance to the coast
lend = 50; % length to which NaN is added to have the same size for satellites points
D = 2;
%====select regional area coordinates
%  lon1x = 152; lon2x = 156; lat1y = -30; lat2y = -24; % australi

%  lon1x = -64; lon2x = -36; lat1y = -8; lat2y = 8; % amaz
lon1x = 0; lon2x = 15; lat1y = -8; lat2y = 8; %gog
% lon1x = -37; lon2x = -33; lat1y = -10; lat2y = -3; % northeast Br


% indlon = find(lon<=lon2x & lon>=lon1x); indlat = find(lat<=lat2y & lat>=lat1y);

% select time format ====
Time = Timetot+datenum('1950-01-01');
[year month day] = datevec(Time);

indt2 = find(Time>=datenum(2016,08,30) & Time<=datenum(2016,09,02)); %austr

% indt2 = find(Time>=datenum(2017,03,06) & Time<=datenum(2017,03,09)); %amz
% indt2 = find(Time>=datenum(2015,09,29) & Time<=datenum(2015,10,16)); %a1
% indt2 = find(Time>=datenum(2017,04,26) & Time<=datenum(2017,05,04)); %a2
% indt2 = find(Time>=datenum(2017,11,15) & Time<=datenum(2017,11,22));% gog

sss_smos = sss_smos(:,:,indt2);
% sssq = squeeze(sss_smos(indlat,indlat,:));

[nx1,ny1,nt]=size(sss_smos);
SSSG_SMOS = []; lon_SMOS = []; lat_SMOS =[]; sss_SMOS =[];  sssa2_SMOS =[]; 
cont = 0; lonsec_mean =[]; latsec_mean = []; 
for ip = 1:nt
    ip
sss_smos_mean = squeeze(sss_smos(:,:,ip));


lon_smos = lon; lat_smos =lat;
%rearrange lon,lat,SSS as vectors
[lat2_smos,lon2_smos]=meshgrid(lat_smos,lon_smos);

[nx,ny]=size(sss_smos_mean);

lon1_smos=reshape(lon2_smos,nx*ny,1);
lat1_smos=reshape(lat2_smos,nx*ny,1);
sss1_smos=reshape(sss_smos_mean,nx*ny,1);
...
load('D:\Dossa\SSS\data\extractions\worldcoastline.mat')
% lonx_cotei = coastlon; 
% latx_cotei = coastlat;
 D=2;
%D=0;
...
% hold on
% plot(lonx_cotei,latx_cotei,'-k','linewidth',0.5)
% 
%%
...
%       m_plot(lonx_norm,laty_norm ,'k')   % plot du crosshore section
    ...
    lonp(1)=lonx_norm(1)-0.25*cosd(ang_coast);
    lonp(2)=lonx_norm(1)+0.25*cosd(ang_coast);
    lonp(3)=lonx_norm(2)+0.25*cosd(ang_coast);
    lonp(4)=lonx_norm(2)-0.25*cosd(ang_coast);
    latp(1)=laty_norm(1)-0.25*sind(ang_coast);
    latp(2)=laty_norm(1)+0.25*sind(ang_coast);
    latp(3)=laty_norm(2)+0.25*sind(ang_coast);
    latp(4)=laty_norm(2)-0.25*sind(ang_coast);
...
save -v7.3 D:\Dossa\SSS\data\SMOS\grad_valids\gradsmos_250km_aus sss_SMOS SSSG_SMOS sssa2_SMOS lon_cst lat_cst SSSG lona lata long_smos lati_smos Timetot lonsecmean latsecmean numbpt npointax lon_css lat_css sss_css

toc % time elapsed 80.078785 seconds ~ 22h50 min
