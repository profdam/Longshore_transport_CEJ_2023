...
[nx1,ny1,nt]=size(ACCESS1_0_hs0p1_1979_2004);
hsG_ENSEMBLE = []; lon_ENSEMBLE = []; lat_ENSEMBLE =[]; hs_ENSEMBLE =[];  hsa2_ENSEMBLE =[]; 
cont = 0; lonsec_mean =[]; latsec_mean = []; 
hs_mean=[]; lon_mean = []; lat_mean = [];
for ip = 1%:nt
    ip;
ENSEMBLE_hs_mean = nanmean(ACCESS1_0_hs0p1_1979_2004,3); %squeeze(ENSEMBLE_hs(:,:,ip));
lon_ENSEMBLE = qlon; lat_ENSEMBLE =qlat;
