...
aaa= nan(size(dm_ac));
aaa(1:size(dm_bc,1),:,:)=dm_bc;
dm_bc=aaa;

aaa= nan(size(tm_ac));
aaa(1:size(tm_bc,1),:,:)=tm_bc;
tm_bc=aaa;

aaa= nan(size(hs_ac));
aaa(1:size(hs_bc,1),:,:)=hs_bc;
hs_bc=aaa;
...
for time = 1 : nhs
HS_2081_2100(:,:,time) = interp2(Xlong, Ylat, HS(:,:,time)', QLON, QLAT);
TM_2081_2100(:,:,time) = interp2(Xlong, Ylat, TM(:,:,time)', QLON, QLAT);
DM_2081_2100(:,:,time) = interp2(Xlong, Ylat, DM(:,:,time)', QLON, QLAT);
...
ENSEMBLE_hs_End_4p5 =permute(HS_2081_2100, [1 3 2]);
ENSEMBLE_tm_End_4p5 =permute(TM_2081_2100, [1 3 2]);
ENSEMBLE_dm_End_4p5 =permute(DM_2081_2100, [1 3 2]);
...
clearvars -EXCEPT HS_hist TM_hist DM_hist...
    ...
    HS_End_4p5 TM_End_4p5 DM_End_4p5 lon_f lat_f
