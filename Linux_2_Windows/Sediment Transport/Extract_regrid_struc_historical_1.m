...
for time = 1 : nhs
HS_1979_2005(:,:,time) = interp2(Xlong, Ylat, HS(:,:,time)', lon, lat,'nearest');
TM_1979_2005(:,:,time) = interp2(Xlong, Ylat, TM(:,:,time)', lon, lat,'nearest');
DM_1979_2005(:,:,time) = interp2(Xlong, Ylat, DM(:,:,time)', lon, lat,'nearest');
...
HS_hist=[];
for i=1:48
hs_select=griddata(Xlong_unstruct,Ylat_unstruct,hs_unstruct(:,i),qlon,qlat,'nearest');
HS_hist=cat(3,HS_hist,hs_select);
end
hs_unstruct=HS_hist;
...
%Extract nearest to the coast
hs_select=griddata(lon_int,lat_int,hs_int,qlon,qlat,'nearest');
tm_select=griddata(lon_int,lat_int,tm_int,qlon,qlat,'nearest');
dm_select=griddata(lon_int,lat_int,dm_int,qlon,qlat,'nearest');
...
for i=1:324
hs_select=griddata(Xlong_unstruct,Ylat_unstruct,hs_giard_unstruct(:,i),qlon,qlat,'nearest');
tm_select=griddata(Xlong_unstruct,Ylat_unstruct,tm_giard_unstruct(:,i),qlon,qlat,'nearest');
dm_select=griddata(Xlong_unstruct,Ylat_unstruct,dm_giard_unstruct(:,i),qlon,qlat,'nearest');
...
lon_int=QLON(ind);
lat_int=QLAT(ind);
HS_hist=griddata(lon_int,lat_int,hs_int,double(qlon),double(qlat),'nearest');
hs_select_struct=cat(3,hs_select_struct,HS_hist);
end
%hs_select_struct(:,:,time) = interp2(Xlong_struct, Ylat_struct, hs_struct(:,:,time)', double(qlon),double(qlat),'nearest');
...
lon_int=QLON(ind);
lat_int=QLAT(ind);
aaa=scatteredInterpolant(Xlong_unstruct,Ylat_unstruct, hs_unstruct(:,1),'nearest');
...
hs_select_unstruct= scatteredInterpolant(Xlong_unstruct, Ylat_unstruct, hs_unstruct(:,1), double(qlon),double(qlat),'nearest');
aaa=scatteredInterpolant(Xlong_unstruct,Ylat_unstruct, hs_unstruct(:,1),'nearest');
